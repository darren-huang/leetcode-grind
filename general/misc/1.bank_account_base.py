...
class BankAccounts:
    ...
    def get(self, ts: int, uuid: str):
        if uuid not in self.accts:
            self._error(ts)
        else:
            print(f"{ts}: {self.accts[uuid]}")

    def deposit(self, ts: int, uuid: str, amt: int):
    ...
    def transfer(self, ts: int, uuid1: str, uuid2: str, amt: int):
        if (
            uuid1 not in self.accts
    ...
    def create(self, ts: int, uuid: str):
        if uuid not in self.accts:
            self.accts[uuid] = 0
